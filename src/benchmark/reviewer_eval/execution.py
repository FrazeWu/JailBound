"""Bounded, offline-preflight execution entrypoints for reviewer evaluation."""
...
def load_local_qwen(resolved: ResolvedModel) -> LocalQwenHandle:
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="auto",
        # FOL needs a second backward pass, unsupported by Flash SDPA kernels.
...
def _run_local_qwen_tensor(
    ...
    if job.method not in _TENSOR_METHODS:
        raise ExecutionError(f"unsupported local tensor method: {job.method}")
    expected_checkpoints = (0,) if job.method == "init" else settings.checkpoints
    ...
    optimizer = (
        _baseline_optimizer(job.method, settings, embedding)
        if job.method in {"pez", "gbda", "gcg"}
        else _tensor_optimizer(job.method, settings)
    )
    snapshots = optimizer.run(
        ...
        _tensor_ledger(job.method, settings),
    ...
    return [
        OptimizationSnapshot(
            ...
            representation=(
                ...
                if job.method == "init"
                else f"tensor_embeddings:{getattr(snapshot, 'selection_branch', job.method)}"
            ),
...
def _prepare_execution(request: ExecutionRequest) -> _PreparedExecution:
    ...
    selected = tuple(sorted(rows, key=lambda row: row.example_id)[: request.requested_limit])
