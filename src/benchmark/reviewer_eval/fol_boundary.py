...
def select_base_radius(
    acceptance: Mapping[float, Sequence[bool]], *, minimum_rate: float = 0.8
) -> float | None:
    """Choose the largest radius meeting a semantic-acceptance rate threshold."""
    ...
    for radius, outcomes in acceptance.items():
        ...
        if sum(outcomes) / len(outcomes) >= minimum_rate:
