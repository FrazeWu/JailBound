...
# basic config (defaults come from environment variables if present)
...
# Optional: 使用兼容 OpenAI 的第三方服务（如七牛云 QnAI GC）时设置自定义 Base URL
_OPENAI_BASE_URL = flags.DEFINE_string(
    "openai_base_url",
    os.getenv("QNAPIURL", ""),
    "Custom OpenAI-compatible API base URL (e.g. https://api.qnaigc.com/v1).",
)
...
def get_args():
    ...
    openai_base_url = _OPENAI_BASE_URL.value
    gpus = _GPUS.value
