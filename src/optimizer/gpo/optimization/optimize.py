...
from optimizer.gpo.dataset.base import Base_Dataset
from src import prompt_utils
from metrics.gpo_eval import eval_utils
from optimizer.gpo.optimization.gradient.base_gradient import Base_Gradient
from optimizer.gpo.optimization.learning_rate.base_lr import Base_LR
from optimizer.gpo.optimization.utilize_gradient.base_util_gradient import Base_Util_Gradient
from optimizer.gpo.optimization.first_order_momentum.base_fo_momentum import (
    Base_FO_Momemtum,
)
from optimizer.gpo.optimization.learning_rate_scheduler.base_lr_scheduler import (
    Base_LR_Scheduler,
)
...
def random_sampling(sample_data_from, all_num_examples, ratio):
    size = max(1, min(int(ratio * all_num_examples), sample_data_from))
    index = np.sort(
        np.array(
            np.random.choice(
                sample_data_from, size=size, replace=False
            )
        )
    )
    return index
...
def prompt_optimization(
    ...
    # Allow any model name — not just gpt-3.5-turbo / gpt-4
    prompt_utils.configure_openai_client(
        api_key=openai_main_api_key if openai_main_api_key else None,
    ...
    if scorer_llm_name in {"gpt-3.5-turbo", "gpt-4"}:
        ...
        # Set chat template for Llama2-chat models if not already set
        if scorer_llm_name in {"llama2-chat-7b", "llama2-chat-13b"} and not tokenizer.chat_template:
            tokenizer.chat_template = "{% for message in messages %}{% if message['role'] == 'user' %}{{ '<s>[INST] ' + message['content'] + ' [/INST]' }}{% elif message['role'] == 'assistant' %}{{ message['content'] + '</s>' }}{% endif %}{% endfor %}"
        ...
        # Generic OpenAI-compatible scorer (any model name, e.g. qwen-72b)
    ...
    for data in data_list:
        ...
        format_index = np.sort(
            np.array(
                np.random.choice(
                    format_num_examples,
                    size=min(format_data_num, format_num_examples),
                    replace=False,
                )
            )
        )
    ...
    for evaluating_data in evaluating_datas:
        ...
        epoch_pbar = tqdm(range(num_search_epochs), desc="Epochs", position=0, leave=True)

        for i_epoch in epoch_pbar:
            tqdm.write(f"\n================== Epoch {i_epoch} =====================")
            optimizer_llm_temperature_curr = optimizer_llm_dict["temperature"]

            step_pbar = tqdm(range(steps), desc=f"Epoch {i_epoch} Steps", position=1, leave=False)
            for i_step in step_pbar:
                tqdm.write(f"================== step {i_step} =====================")
