...
try:
    ...
    # Fallback: use rouge_score if the 'rouge' package is unavailable
    from rouge_score import rouge_scorer as _rouge_scorer_mod
    class Rouge:
        """Thin shim over rouge_score to match the rouge.Rouge() API."""
        def __init__(self):
            self._scorer = _rouge_scorer_mod.RougeScorer(['rougeL'], use_stemmer=False)
        def get_scores(self, pred, ref):
            result = self._scorer.score(ref, pred)
            f = result['rougeL'].fmeasure
            return [{'rouge-l': {'f': f, 'p': result['rougeL'].precision, 'r': result['rougeL'].recall}}]
import pdb
