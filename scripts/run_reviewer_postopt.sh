...
CUDA_VISIBLE_DEVICES=0 uv run python scripts/freeze_semantic_calibration.py \
  ...
  > outputs/reviewer-semantic-calibration.log 2>&1

CUDA_VISIBLE_DEVICES=0 uv run python scripts/run_reviewer_experiments.py materialize \
  --config configs/benchmark/reviewer_additions.yaml \
  --output-root "$output_root" \
  --final-only \
  > outputs/reviewer-materialize.log 2>&1
...
CUDA_VISIBLE_DEVICES=0 uv run python scripts/run_reviewer_experiments.py run-target \
  --config configs/benchmark/reviewer_additions.yaml \
  --output-root "$output_root" \
  --target qwen2_5_7b \
  > outputs/reviewer-target-qwen7b.log 2>&1
