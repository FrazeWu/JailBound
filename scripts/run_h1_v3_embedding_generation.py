"""Batch-generate H1-v3's accepted new-radius embedding perturbations."""
...
def main() -> int:
    ...
    if args.batch_size < 1 or args.shard_count < 1 or not 0 <= args.shard_index < args.shard_count:
        raise ValueError("H1-v3 batch and shard settings are invalid")
    config = load_h1_v3_config(args.config)
    if args.source not in config.h1_v3.sources:
        raise ValueError("H1-v3 embedding generation requested an unconfigured source")
    paths = H1V3Paths(ROOT / config.h1_v3.source_root, ROOT / config.h1_v3.output_root)
    try:
        run = json.loads((paths.output_root / "run_manifest.json").read_text(encoding="utf-8"))
        run_id, config_hash = run["run_id"], run["config_hash"]
    except (KeyError, OSError, json.JSONDecodeError) as error:
        raise ValueError("H1-v3 run manifest is invalid") from error
    grouped: dict[str, list[dict[str, object]]] = defaultdict(list)
    for row in read_jsonl(paths.selected_directions()):
        if row.get("source") != args.source:
            continue
        sample_id = row.get("sample_id")
        if not isinstance(sample_id, str) or not sample_id:
            raise ValueError("H1-v3 selected direction has an invalid sample identity")
        grouped[sample_id].append(dict(row, kind="perturbation"))
    if not grouped:
        raise ValueError("H1-v3 selected direction ledger is empty for the requested source")
    sample_ids = tuple(
        sample_id for index, sample_id in enumerate(sorted(grouped))
        if index % args.shard_count == args.shard_index
    )
    model_path = config.base.models.surrogate.local_path
    if model_path is None:
        raise ValueError("H1-v3 embedding generation requires a local target")
    ...
    ledger = JsonlLedger(
        paths.response_ledger(config.base.models.targets[0].key, args.source),
        key_fields=("sample_id", "checkpoint"),
    )
    written = failed = 0
    try:
        if handle.model is None or handle.tokenizer is None:
            raise ValueError("H1-v3 local target did not load")
        examples, optimized = frozen_h1_v2_records(paths.source_root, args.source)
            ...
            for start in range(0, len(pending), args.batch_size):
                ...
                for row, state, result, error in zip(chunk, states, generated, errors, strict=True):
                    ...
                    if not isinstance(perturbation_id, str):
                        raise ValueError("H1-v3 perturbation identity is invalid")
                    common = {
                    ...
                    if result is None:
                        record = ResponseRecord(
                            **common, response="", input_tokens=0, generated_tokens=0,
                            ...
                            failure_reason=f"H1-v3 embedding generation error: {type(error).__name__}",
                        )
                    else:
                        record = ResponseRecord(
