"""Bounded command line entrypoint for reviewer-eval artifacts."""
...
from benchmark.reviewer_eval.pipeline import materialize_records_from_disk
from benchmark.reviewer_eval.pipeline import generate_materialized_records, judge_response_records
from benchmark.reviewer_eval.io import read_jsonl
from benchmark.reviewer_eval.judging import OctopusLocalJudge, Qwen32CompatJudge, thresholds
from benchmark.reviewer_eval.runner import SerialTargetBarrier
from benchmark.reviewer_eval.result_aggregation import (
...
from benchmark.reviewer_eval.schema import MaterializationRecord, RecordStatus, ResponseRecord, JudgmentRecord
...
def _run_manifest_builder(config_path: Path, candidate_pool: int) -> int:
    command = [
        sys.executable,
        str(REPOSITORY_ROOT / "scripts" / "build_reviewer_manifests.py"),
        "--config",
        str(config_path),
        "--candidate-pool",
        str(candidate_pool),
...
def _final_materializations(config: ExperimentConfig, output_root: Path) -> list[dict[str, object]]:
    ...
    expected = len(config.data.sources) * len(config.optimization.methods) * config.data.samples_per_source
    if len(records) != expected:
        raise ValueError("final materialization count does not match the locked matrix")
    return records


def _responses_for_target(config: ExperimentConfig, output_root: Path, target_key: str) -> list[dict[str, object]]:
    records: list[dict[str, object]] = []
    for source in config.data.sources:
        for method in config.optimization.methods:
            checkpoint = 0 if method == "init" else 100
...
def _run_target(config: ExperimentConfig, output_root: Path, target_key: str) -> dict[str, object]:
    ...
    materializations = _final_materializations(config, output_root)
    ...
    responses = _responses_for_target(config, output_root, target_key)
    ...
    primary_counts = [
        _judgment_count(config, output_root, judge_key=primary.key, target_key=target_key, threshold=threshold)
        for threshold in thresholds(primary.threshold, primary.threshold_offsets)
    ]
    secondary_counts = [
        _judgment_count(config, output_root, judge_key=secondary.key, target_key=target_key, threshold=threshold)
        for threshold in thresholds(secondary.threshold, secondary.threshold_offsets)
    ]
    response_count = len(responses)
    base_primary = _judgment_count(config, output_root, judge_key=primary.key, target_key=target_key, threshold=primary.threshold)
    base_secondary = _judgment_count(config, output_root, judge_key=secondary.key, target_key=target_key, threshold=secondary.threshold)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "run-target":
            print(json.dumps(_run_target(config, output_root, args.target), sort_keys=True))
            return 0
