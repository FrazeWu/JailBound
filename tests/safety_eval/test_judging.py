...
from benchmark.safety_eval.judging import (
    JudgmentLedger,
    Qwen32CompatJudge,
    normalize_octopus,
    thresholds,
...
def test_octopus_retains_a_valid_score_when_decoded_tag_is_noncanonical() -> None:
    result = normalize_octopus(safe_probability=0.2, decoded_tag="unparseable")
...
def test_compat_judge_derives_label_from_score_and_requested_threshold() -> None:
    client = httpx.Client(
        transport=httpx.MockTransport(
            lambda request: httpx.Response(
                200,
                json={"choices": [{"message": {"content": '{"unsafe_score": 0.8, "unsafe_label": false, "refusal_label": null}'}}]},
            )
        )
    )
    judge = Qwen32CompatJudge(endpoint="http://localhost:8001/v1", client=client)

    record = judge.judge_response(
        run_id="run:test", config_hash="a" * 64, sample_id="sample:test", source="fixture",
        method="init", target_key="qwen2_5_7b", target_revision="local:test",
        response_text="A benign fixture response.", threshold=0.5,
    )
