...
def test_calibration_pair_pool_replaces_only_rejected_holdouts() -> None:
    rejected = CalibrationCandidate("source:001", "source", "category_a", "fixture rejected")
    rejected_negative = CalibrationCandidate("source:002", "source", "category_a", "fixture negative one")
    accepted = CalibrationCandidate("source:003", "source", "category_a", "fixture accepted")
    accepted_negative = CalibrationCandidate("source:004", "source", "category_a", "fixture negative two")
    ...
    pairs = build_calibration_pairs_from_pool(
        (
            CalibrationHoldout(rejected, rejected_negative),
            CalibrationHoldout(accepted, accepted_negative),
        ),
        per_source=1,
        paraphrase=paraphrase,
        category_for_text=lambda text: "category_a" if text.endswith("fixture accepted") else "category_b",
        entities_preserved=lambda _, __: True,
        similarity=lambda _, __: 0.9,
        max_attempts=1,
    )

    assert len(pairs) == 2
    assert paraphrased == ["fixture rejected", "fixture accepted"]


def test_calibration_pair_pool_replaces_nonfinite_similarity_holdout() -> None:
    invalid = CalibrationCandidate("source:001", "source", "category_a", "fixture invalid")
    invalid_negative = CalibrationCandidate("source:002", "source", "category_a", "fixture invalid negative")
    accepted = CalibrationCandidate("source:003", "source", "category_a", "fixture accepted")
    accepted_negative = CalibrationCandidate("source:004", "source", "category_a", "fixture accepted negative")

    pairs = build_calibration_pairs_from_pool(
        (
            CalibrationHoldout(invalid, invalid_negative),
            CalibrationHoldout(accepted, accepted_negative),
        ),
        per_source=1,
        paraphrase=lambda text: f"rewrite:{text}",
        category_for_text=lambda _: "category_a",
        entities_preserved=lambda _, __: True,
        similarity=lambda left, _: float("nan") if left == "fixture invalid" else 0.9,
        max_attempts=1,
    )

    assert [(pair.positive, pair.similarity) for pair in pairs] == [(True, 0.9), (False, 0.9)]


def test_calibration_pair_pool_exhaustion_reports_aggregate_rejection_counts() -> None:
    candidate = CalibrationCandidate("source:001", "source", "category_a", "fixture rejected")
    negative = CalibrationCandidate("source:002", "source", "category_a", "fixture negative")

    with pytest.raises(ValueError) as exc_info:
        build_calibration_pairs_from_pool(
            (CalibrationHoldout(candidate, negative),),
            per_source=1,
            paraphrase=lambda _: "wrong category",
            category_for_text=lambda _: "category_b",
            entities_preserved=lambda _, __: True,
            similarity=lambda _, __: 0.9,
            max_attempts=1,
        )

    assert exc_info.value.accepted_by_source == {"source": 0}
    assert exc_info.value.rejected_paraphrase_by_source == {"source": 1}
    assert exc_info.value.rejected_similarity_by_source == {"source": 0}
